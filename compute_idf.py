...
if __name__ == '__main__':
    ...
    if args.output is None:
        args.output = f'{args.subset}_idf.pkl'
    
    print(f'Loading dataset {args.dataset}...')
    dataset = datasets.load_dataset(path=args.dataset, name=args.subset, split='test')
    print(f'Loading tokenizer {args.tokenizer}...')
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer)

    print(f'Creating IDF dict for {args.subset}...')
    idf_dict = get_idf_dict(arr=dataset['canonical_solution'], tokenizer=tokenizer, nthreads=args.nthreads)
